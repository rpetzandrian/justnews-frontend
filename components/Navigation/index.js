  ...
  return (
    <>
      <Navbar
        bg="light"
        expand="lg"
        className="border border-bottom py-2 px-4 ff-lato"
      >
        <Navbar.Brand
          href="/"
          className="ms-3 ff-newsreader fw-700 display-1 app-name"
        >
          Just News
        </Navbar.Brand>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="d-flex mx-auto">
            <Nav.Link href="/" className={`mx-5 text-muted ${asActive("")}`}>
              Home
            </Nav.Link>
            <Nav.Link
              href="/articles"
              className={`mx-5 text-muted ${asActive("articles")}`}
            >
              Articles
            </Nav.Link>
            <Nav.Link
              href="/category"
              className={`mx-5 text-muted ${asActive("category")}`}
            >
              Category
            </Nav.Link>
            <Nav.Link
              href="/about"
              className={`mx-5 text-muted ${asActive("about")}`}
            >
              About
            </Nav.Link>
          </Nav>
          {!loggedOut ? (
            <div className="d-flex align-items-center">
              <div className="mx-3 input-group">
                <img
                  className="input-group-text ps-4 group-text"
                ...
                <input
                  placeholder="Search"
                  type="text"
                  className="form-control py-3 px-4 form-input-group profile"
                  onFocus={() => router.push("/search")}
                  onClick={(e) =>
                    changeKeyword({ ...query, search: e.target.value })
                  }
                />
              ...
              <div className="mx-3 notification">
                <img
                  src={`/icons/${
                    notif?.unreadNotif > 0 ? "notif-read" : "notif"
                  }.svg`}
                  als="notif"
                  onClick={() => {
                    setNotifShow(!notifShow);
                    notif?.unreadNotif > 0 ? readNotif() : null;
                  }}
                />
              </div>
