...
const tag_list = ["covid", "cripto", "invest", "taliban"];

export default function Home({ latest, recommended }) {
  const { auth, loggedOut, mutateAuth } = useAuth();
  const { category } = useCategory();
  const { latest: latestPosts, mutateLatest } = useLatest(auth?.data?.id, {
    initialData: latest,
  });
  const { recommended: recommendedPosts, mutateRecommended } = useRecommended(
    auth?.data?.id,
    ...
    <>
      ...
      <section className="bg-light p-5 rounded-lg jumbotron">
        <div className="mt-5 py-5">
          <h1 className="display-2 ff-newsreader fw-700">
            Share Information and <br></br>Educate People
          </h1>
          <p
            className="lead w-50 ff-lato text-shadow"
            style={{ fontSize: "24px" }}
          >
            ...
            return (
              <div className="mx-2 figure">
                <img
                  width={190}
                  height={200}
                  src={`${process.env.img_url}${e?.image}`}
                />
      ...
      <section className="px-5 mt-5 recommended">
        ...
        <div className="d-flex mt-4 overflow-auto tags">
          {loggedOut ? (
            recommended?.post?.length < 1 ? (
              <div>Post Not Found!</div>
            ) : (
              recommended?.post?.map((e) => {
          ...
          {recommendedPosts?.post?.length < 1 ? (
            <div>Post Not Found!</div>
          ) : (
            recommendedPosts?.post?.map((e) => {
      ...
      <section className="p-5 mt-5 rounded-lg bg-info middle-jumbotron">
        <Row className="p-5 align-items-center">
          <Col xs={12} md={6}>
            <h3 className="display-5 ff-newsreader">
              Let's hear about Kayla's success story
            </h3>
            <p className="fs-4 ff-lato fw-light">
      ...
      <section className="p-5 mt-5 latest-news">
        ...
        <div className="d-flex justify-content-start flex-wrap mt-3">
          ...
          {latestPosts?.post?.length < 1 ? (
            <div>Post Not Found!</div>
          ) : (
            latestPosts?.post?.map((e) => {
...
export async function getStaticProps() {
  const resultLatest = await axios.get(
    `${process.env.api_url}/posts?time=desc`,
    {
      headers: {
  ...
  const resultRecommended = await axios.get(
    `${process.env.api_url}/posts?recomended=true`,
    {
      headers: {
        Origin: "http://localhost:3000",
      },
